...
```{r markers-in-space, cache=T, fig.width=8, fig.height=6, fig.align='center'}
y <- log(exprs(HSMM[cont_genes,]) + 1)
y <- scale(t(y))
y <- data.frame(y)
M_y <- cbind(M, y)
...
ggplot(M_y_melted, aes(x=component_1, y=component_2, size=2*count, color=factor(monocle_state))) + 

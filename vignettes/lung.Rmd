...
set.seed(123)
lsce <- cluster_embedding(lsce, method='kmeans', k = 4)
plot_embedding(lsce)
...
lsce_bp_at1 <- lsce[, pData(lsce)$cluster %in% c(1,3) ]
...
plot_pseudotime_density(lsce_bp_at1)
```

### BP to AT2 transition

```{r ps-2, cache=TRUE, fig.align='center'}
lsce_bp_at2 <- lsce[, pData(lsce)$cluster %in% c(1,2)]

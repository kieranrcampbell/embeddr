...
sce_13_tmp <- sce_13
phenoData(sce_13_tmp)$cluster <- plyr::mapvalues(pData(sce_13_tmp)$cluster,
                                                 from=1:3,
                                                 to=c('BP','AT2','AT1'))

plot_embedding(sce_13_tmp)
```
...
plot_pseudotime_model(sce_bp_at1[gene,], null_model) + ggtitle('Fabp5 expression under null model')
...
pseudotime_markers_at1 <- names(gene_classes[gene_classes %in% c(1,2)])
...
library(gplots)
library(NMF)
prev_identified_1 <- pseudotime_markers_at1 %in% markers1
prev_identified_2 <- pseudotime_markers_at2 %in% markers2
all_pseudotime_markers <- c(pseudotime_markers_at1[prev_identified_1], pseudotime_markers_at1[!prev_identified_1],
                            pseudotime_markers_at2[prev_identified_2], pseudotime_markers_at2[!prev_identified_2])

previously_identified <- c(rep('Prev', sum(1*prev_identified_1)),
                           rep('New', sum(!prev_identified_1)),
                           rep('Prev', sum(1*prev_identified_2)),
                           rep('New', sum(!prev_identified_2)))
# prev_colors <- plyr::mapvalues(previously_identified, from=c('Prev','New'), to=c('darkgrey','lightgrey'))

cell_type <- pData(sce_13)$putative_cell_type[order(pseudotime(sce_13))]
# cell_colors <- plyr::mapvalues(cell_type, from=c('AT1','BP','AT2'), to=c('orange','black','darkred'))

mm <- exprs(sce_13[all_pseudotime_markers,order(pseudotime(sce_13))])
...
aheatmap(mm, Rowv = NA, Colv = NA,
         annRow = data.frame(New = previously_identified),

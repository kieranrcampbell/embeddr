...
W <- weighted_graph(x, metric='correlation')
```
...
ncv <- 8
inds <- createFolds(1:nrow(Me), ncv)

embeddings <- lapply(1:ncv, function(i) {
  xep <- xe[,-inds[[i]] ]
  W_tmp <- weighted_graph(xep, metric='correlation', nn = 6)
  M_tmp <- laplacian_eigenmap(W_tmp, type='unorm')
  M_tmp$cluster <- Me$cluster[-inds[[i]] ]
  M_tmp$fold <- i
  M_tmp
  })

df_emb <- do.call('rbind', embeddings)
plot_embedding(df_emb) + facet_wrap(~ fold)
```
...
inds_3 <- createFolds(1:nrow(Me), k=ncv)

embeddings_3 <- lapply(1:ncv, function(i) {
  xep <- xe[,-inds_3[[i]] ]
  W_tmp <- weighted_graph(xep, metric='correlation', nn = 6)
  M_tmp <- laplacian_eigenmap(W_tmp, type='unorm')
  M_tmp$cluster <- Me$cluster[-inds_3[[i]] ]
  M_tmp$fold <- i
  M_tmp
  })

df_emb3 <- do.call('rbind', embeddings_3)
plot_embedding(df_emb3) + facet_wrap(~ fold)
```
...
folds <- createMultiFolds(1:nrow(Me), k = 8, times = 30)
all_embeddings <- lapply(folds, function(ind) {
    xep <- xe[,ind]
    W_tmp <- weighted_graph(xep, metric='correlation', nn = 6)
    M_tmp <- laplacian_eigenmap(W_tmp, type='unorm')
    M_tmp$cluster <- Me$cluster[ind]
    M_tmp
  })

#all_embeddings_unlist <- unlist(all_embeddings, recursive=FALSE)

psts <- lapply(all_embeddings, function(M_tmp) {
  M_tmp <- fit_pseudotime(M_tmp, clusters = c(1,3))
  M_tmp$pseudotime
})

 corrs <- sapply(1:length(all_embeddings), function(i) {
  cells_in_i <- all_embeddings[[i]]$cell_id
  Mp_i <- filter(Mp, cell_id %in% cells_in_i)
  abs(cor(psts[[i]], Mp_i$pseudotime))
})

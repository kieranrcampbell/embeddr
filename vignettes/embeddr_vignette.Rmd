...
W <- weighted_graph(sce_embedding, nn = 4, metric='correlation')
sce <- laplacian_eigenmap(sce, W)
...
sce <- cluster_embedding(sce, method='mm', k=3)
plot_embedding(sce)
```
...
plot_in_pseudotime(sce_23, genes_to_plot, gene_short_names)
```
...
print(table(dplyr::select(pData(sce), monocle_classification, embeddr_classification)))
```
...
Model a signle gene in pseudotime:

```{r model_pseudotime, cache=TRUE, fig.align='center'}
...
plot_pseudotime_model(sce_23, model, cdk1)

```

Now try all of them:

```{r all-genes, cache=TRUE}
...
sig_tests <- sapply(featureNames(sce_23_kept), test_gene, sce_23_kept)

```

And plot p-vals:

```{r plot-p, cache=TRUE, fig.align='center'}
save(sig_tests, file='/net/isi-scratch/kieran//embeddr/embeddr/data/sig_tests.Rdata')
qplot(sig_tests, binwidth=0.01) + theme_bw() + xlab('p-value') 
```

correct p-values and replot:

```{r correct-p, cache=TRUE, fig.align='center'}
adj_sigs <- p.adjust(sig_tests, method='BH')
qplot(adj_sigs, binwidth = 0.01) + theme_bw() + xlab('corrected p-value')
alpha <- 0.01
sig_inds <- adj_sigs < alpha
sig_genes <- names(which(sig_inds))
print(length(sig_genes))

```

```{r correlate-genes, fig.width=9, fig.height=5, fig.align='center', cache=TRUE}
exprs_cor <- exprs(sce_23[sig_genes, ])
gene_cor <- cor(t(exprs_cor))
d <- 1 - gene_cor / 2

hc <- hclust(as.dist(d))
co <- order.optimal(as.dist(d), hc$merge)$order
po <- order(pData(sce_23)$pseudotime)
exprs_ordered <- exprs_cor[co, po] 

#image(exprs_ordered, col=colorRamps::blue2red(2))

exprs_ordered <- t(scale(t(exprs_ordered)))
rownames(exprs_ordered) <- NULL
heatmap.2(exprs_ordered, Rowv=TRUE, Colv=NULL, dendrogram='row', density.info='none', keysize=1,
          trace='none', sepwidth=c(0,0), breaks=seq(from=-1, to=1, by=0.1), 
          col=colorRampPalette(c('dodgerblue3','white','brown3'))(20)) #matlab.like(20))

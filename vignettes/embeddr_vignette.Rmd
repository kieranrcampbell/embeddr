...
diff_gene_test <- pseudotime_test(sce_23_kept)
print(head(diff_gene_test))
```
...
```{r predicted-expression, cache=TRUE}
...
# co <- order.optimal(as.dist(dist_cor), hc$merge)$order
# po <- order(pData(sce_23)$pseudotime)
# exprs_ordered <- pcor[co, po] 
# 
...
```{r plot-modules, fig.width=8, fig.height=9, cache=TRUE}
n_cuts <- 5
...
enriched_terms <- apply(genes_per_class, 2, function(gene_set) {
  ...
  go$qval <- p.adjust(go$over_represented_pvalue, method='BH')
  go <- dplyr::filter(go, qval < 0.01)
  return(dplyr::select(go, category, qval, term))
  })
...
kable(reduced[[1]])#, format='markdown')
```
